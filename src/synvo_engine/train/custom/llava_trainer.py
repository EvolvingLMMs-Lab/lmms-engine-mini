...
from transformers.trainer import ALL_LAYERNORM_LAYERS, logger
...
class LLaVATrainer(Trainer):
    ...
    def _maybe_log_save_evaluate(
        self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval, mfu=None
    ):
        if (
            ...
            peak_mem, mem = self.get_memory()
            logs["peak_memory (GB)"] = round(peak_mem, 1)
            logs["memory (GB)"] = round(mem, 1)
    ...
    def training_step(
        ...
        if self.state.global_step == 0 or getattr(self, "cur_time", None) is None:
            ...
            self.flops = 0
        if (
            self.step % self.args.gradient_accumulation_steps == 0
            ...
            # sp_size = DIST_ENV.distributed_sequence_parallel_size
            sp_size = 1
            flops_tensor = torch.tensor(self.flops / sp_size, device=device)
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            # trace_name = f"profile_{dist.get_rank()}_" + str(p.step_num) + ".pt.trace.json"
            # trace_path = os.path.join("./", trace_name)
            # p.export_chrome_trace(trace_path)

            # p = torch.profiler.profile(
            # activities=(torch.profiler.ProfilerActivity.CPU, torch.profiler.ProfilerActivity.CUDA),
            # schedule=torch.profiler.schedule(**self.args.profiler_schedule_kwargs),
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # inputs["modalities"],
                # inputs["ids"],
                # ) = broadcast_obj_across_sequence_parallel_group(
                # [
                # inputs["input_ids"],
                # inputs["attention_mask"],
                # inputs["images"],
                ...
                # inputs["images"][i] = inputs["images"][i].cuda()
                ...
                if (
                    ...
                    if tr_loss.device != tr_loss_step.device:
                        raise ValueError(
                            f"loss must be on original device: {tr_loss.device} but in use is {tr_loss_step.device}"
                        )
